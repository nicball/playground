...
void render_ascii(const uint8_t* const inbuf, const int inbuf_len, uint8_t* const outbuf) {
  ...
  for (i = 0; i + 16 < inbuf_len; i += 16) {
    __m128i byte = _mm_loadu_si128(&inbuf[i]);
    __m128i ascii = _mm_blendv_epi8(
      byte,
      _mm_set1_epi8('.'),
      _mm_and_si128(
        _mm_cmpgt_epi8(byte, _mm_set1_epi16(' ' - 1)),
        _mm_cmpgt_epi8(_mm_set1_epi8(0x7e + 1), byte)));
    _mm_storeu_si128(&outbuf[i], ascii);
  }
  for(; i < inbuf_len; ++i)
    outbuf[i] = ' ' <= inbuf[i] && inbuf[i] <= 0x7e ? inbuf[i] : '.';
}
...
void render_hex(
  ...
  for (base = 0; base + 8 < inbuf_len; base += 8) {
    __m128i byte = _mm_cvtepu8_epi16(_mm_loadu_si64(&inbuf[base]));
    ...
    __m128i out_full_idx = _mm_loadu_si128(&o2n[out_off]);
    ...
    _mm_storeu_si128(&outbuf[out_off], out_full);
    __m128i out_rest_idx = _mm_loadu_si128(&o2n[out_off + 16]);
    ...
    uint64_t nnib = _mm_cvtsi128_si64(nib);
