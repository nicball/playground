...
void parse_dump(const int len, const char** args, dump_args_t* result) {
  ...
  result->num_columns = 32;
  result->input_fd = 0;
  result->output_fd = 1;
  for (int i = 0; i < len; ++i) {
    if (got_positionals < sizeof(positionals) / sizeof(positionals[0])) {
...
int render_xxd(const char* const restrict inbuf, const int inbuf_len, const int num_columns, const int group_size, size_t offset, char* restrict outbuf) {
  ...
  for (int inbase = 0; inbase < inbuf_len; inbase += num_columns) {
    ...
    if (r == num_columns) {
      outbuf[outbase + 9] = ' ';
      render_fixed32_hex(&inbuf[inbase], &outbuf[outbase + 10]);
      cursor = outbase + line_width - num_columns - 1;
      outbuf[cursor - 2] = ' ';
      outbuf[cursor - 1] = ' ';
    }
    else {
    ...
    if (r == num_columns) {
      render_fixed32_ascii(&inbuf[inbase], &outbuf[cursor]);
    }
    else {
      for (int i = 0; i < r; ++i) {
